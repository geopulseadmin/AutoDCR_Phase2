...
//           let entry_timestamp = item[header] ? moment(item[header]) : null;

//           if (entry_timestamp && entry_timestamp.isValid()) {
//             // Format the date for display
//             cell.textContent = entry_timestamp.format('DD/MM/YYYY HH:mm');
//             // Store the raw date value for sorting
//             cell.setAttribute('data-sort', entry_timestamp.toISOString());
//           } else {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Close box when clicking outside of it or outside the button, but not on the map
  document.addEventListener('click', function (event) {
    // Check if the target is outside the box and button, and is NOT on the map
    const isMapClick = event.target.classList.contains('leaflet-container'); // Modify based on your map library

    if (!box.contains(event.target) && !button.contains(event.target) && !isMapClick) {
      box.style.display = 'none';
    }
  });
